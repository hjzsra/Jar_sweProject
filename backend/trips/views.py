...
from .permissions import IsApprovedDriver, IsTripDriver
...
from .services import TripService, GoogleMapsService
                    ...
                    "matching_trips": [
                        {
                            ...
                            "driver": {
                                ...
                                "profile_picture": trip.driver.profile_picture.url
                            ...
                            "pickup_distance_km": getattr(
                                trip, "pickup_distance", None
                            ),
                            "dropoff_distance_km": getattr(
                                trip, "dropoff_distance", None
                            ),
                        }
...
class JoinRideView(APIView):
    ...
    def post(self, request, trip_id):
        ...
        if trip.status != "scheduled":
            return Response(
                {"success": False, "message": "Trip is not available for joining"},
...
class AvailableRidesMapView(APIView):
    ...
    def get(self, request):
        ...
        trips_query = (
            Trip.objects.filter(status="scheduled", available_seats__gt=0)
                ...
                nearby_trips.append(
                    {
                        ...
                        if hasattr(trip.driver, "driver_profile")
                        else None,
    ...
    def post(self, request, trip_id, passenger_id):
        try:
            ...
            trip_passenger = TripPassenger.objects.get(
                trip=trip, passenger_id=passenger_id, status="pending"
            )
...
class RejectPassengerView(APIView):
    ...
    def post(self, request, trip_id, passenger_id):
        ...
        return Response(
            {"success": True, "message": "Passenger request rejected"}
        )
...
class TripDetailView(APIView):
    ...
    def get(self, request, trip_id):
        try:
            trip = (
                Trip.objects.select_related("driver", "driver__driver_profile")
                .prefetch_related("trip_passengers__passenger")
                .get(trip_id=trip_id)
            )
        except Trip.DoesNotExist:
            return Response(
...
class CompleteTripView(APIView):
    ...
    def post(self, request, trip_id):
        ...
        if trip.status != "active":
            return Response(
                {"success": False, "message": "Trip is not active"},
                status=status.HTTP_400_BAD_REQUEST,
            )
...
class CancelTripView(APIView):
    ...
    def post(self, request, trip_id):
        ...
        if not (is_driver or is_passenger):
            return Response(
                {"success": False, "message": "You are not part of this trip"},
        ...
        if is_driver:
            ...
            try:
                ...
                logger.error(
                    f"Failed to send cancellation notifications: {str(e)}"
                )
            ...
            try:
                ...
                try:
                    ...
                    NotificationService.send_passenger_cancelled_notification(
                        trip, request.user
                    )
                except Exception as e:
                    logger.error(
                        f"Failed to send cancellation notification: {str(e)}"
                    )
...
class MyTripsView(APIView):
    ...
    def get(self, request):
        ...
        if role in ["driver", "all"]:
            driver_trips = Trip.objects.filter(driver=request.user).select_related(
                "driver"
            ).prefetch_related("trip_passengers__passenger")
            if trip_status:
...
class LocationViewSet(viewsets.ModelViewSet):
    ...
    def get_object(self):
        user_location, _ = UserLocation.objects.get_or_create(user=self.request.user)
    ...
    def set_permission(self, request):
        ...
        user_location.location_permission_granted = serializer.validated_data[
            "location_permission_granted"
        ]
        user_location.location_permission_type = serializer.validated_data[
    ...
    def update_location(self, request):
        ...
        transaction.on_commit(
            lambda: process_location_update.delay(
    ...
    def nearby_drivers(self, request):
        ...
        nearby_users = user_location.get_nearby_users(
            radius_km=radius_km, user_type="driver"
        )
        ...
        nearby_users = nearby_users.filter(
            user__role="driver",
            user_driver_profile_is_verified=True,
            user_driver_profile_is_available=True,
        )
    ...
    def nearby_passengers(self, request):
        ...
        nearby_users = user_location.get_nearby_users(
            radius_km=radius_km, user_type="passenger"
        )
